<?php
class ModelSaleMobSMS extends Model {
	public function getmobsmsReports($data) {
		
		$sql = "SELECT * FROM " . DB_PREFIX . "mobsms_report";
		
		$implode = array();
		
		if (isset($data['filter_id']) && !is_null($data['filter_id'])) {
			$implode[] = "mobsms_report_id = '" . $this->db->escape($data['filter_id']) . "'";
		}
		
		if (isset($data['filter_source']) && !is_null($data['filter_source'])) {
			$implode[] = "mobsms_source LIKE '%" . $this->db->escape($data['filter_source']) . "%'";
		}
		
		if (isset($data['filter_destination']) && !is_null($data['filter_destination'])) {
			$implode[] = "mobsms_destination LIKE '%" . $this->db->escape($data['filter_destination']) . "%'";
		}
		
		if (isset($data['filter_message']) && !is_null($data['filter_message'])) {
			$implode[] = "mobsms_message LIKE '%" . $this->db->escape($data['filter_message']) . "%'";
		}
		
		if (isset($data['filter_message_type']) && !is_null($data['filter_message_type'])) {
			$implode[] = "mobsms_message_type = '" . $this->db->escape($data['filter_message_type']) . "'";
		}
		
		if (isset($data['filter_server_status']) && !is_null($data['filter_server_status'])) {
			$implode[] = "mobsms_server_status LIKE '%" . $this->db->escape($data['filter_server_status']) . "%'";
		}
		
		if (isset($data['filter_start_date']) && !is_null($data['filter_start_date'])) {
			$implode[] = "mobsms_sent_on >= '" . $this->db->escape($data['filter_start_date']) . "'";
		}
		
		if (isset($data['filter_end_date']) && !is_null($data['filter_end_date'])) {
			$implode[] = "mobsms_sent_on <= '" . $this->db->escape($data['filter_end_date']) . "'";
		}
